...
import { PageContainer, Card, Modal, ModalHeader, ModalTitle } from '@/components/ui';
...
const StatCard: React.FC<{
  ...
  loading?: boolean;
}> = ({ label, value, change, type, color, icon, loading }) => {
  ...
  const { bg, text } = colorClasses[color] || colorClasses.indigo;

  if (loading) {
    return <Card className="h-40 animate-pulse" padding="lg" radius="2xl" />;
  }

  return (
    <Card className="group hover:shadow-xl hover:shadow-indigo-50/50 transition-all duration-500 overflow-hidden relative" padding="lg" radius="2xl">
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 transition-transform group-hover:scale-110 duration-500 ${bg} ${text}`}>
        {icon}
      </div>
      <p className="text-slate-400 text-sm font-bold mb-1 uppercase tracking-widest">{label}</p>
      <div className="flex items-end gap-3">
        <span className="text-3xl font-black text-slate-800 tracking-tight">{value}</span>
        <span className={`text-xs font-bold mb-1.5 ${type === 'up' ? 'text-emerald-500' : 'text-rose-500'}`}>
          {change}
        </span>
      </div>

      {/* Background decoration */}
      <div className={`absolute -right-4 -bottom-4 w-24 h-24 opacity-[0.03] rotate-12 group-hover:scale-125 transition-transform duration-700 ${text}`}>
        {icon}
      </div>
    </Card>
  );
  ...
  return (
    <PageContainer className="animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-3 bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm relative">
          ...
          <button
            ...
            className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${
              timeRange === 'week' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-slate-600'
            }`}
          >
            本周
          </button>
          <button
            ...
            className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${
              timeRange === 'month' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-slate-600'
            }`}
          >
            本月
          </button>
          ...
          <button
            ...
            className={`p-2 transition-colors rounded-lg ${
              timeRange.startsWith('custom') ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400 hover:text-indigo-600'
            }`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {isLoading
          ? [...Array(4)].map((_, i) => <StatCard key={i} label="" value="" change="" type="up" color="indigo" icon={null} loading />)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 flex flex-col relative" padding="lg" radius="3xl">
        ...
        {/* Retention Chart */}
        <Card className="flex flex-col relative" padding="lg" radius="3xl">
          <div className="mb-10">
          ...
          <div className="mt-8 space-y-4">
            <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-black text-indigo-600">{timeRange === 'week' ? '75.2%' : '72.4%'}</span>
              </div>
      ...
      <Modal isOpen={isCalendarOpen} onClose={() => setIsCalendarOpen(false)} size="md">
        <ModalHeader icon={<CalendarIcon className="w-6 h-6" />}>
          <ModalTitle subtitle="选择需要分析的项目周期">自定义时间段</ModalTitle>
        </ModalHeader>
