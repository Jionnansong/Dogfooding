...
import { TrendingUp, Users, Play, Share2, Calendar as CalendarIcon, Loader2, ArrowRight, Check } from 'lucide-react';
import { useGetAnalyticsDataQuery } from '@/store/slices/apiSlice';
import { Card, PageHeader, Modal, ModalActions, ModalButton, FormField, Input } from '@/components/ui';
...
const DataBoard: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <PageHeader
        title="数据看板"
        subtitle={`作品多维表现深度洞察 (${displayRange})`}
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        ) : data?.summary.map((stat: { label: string; value: string; change: string; type: 'up' | 'down'; color: string }, idx: number) => (
      ...
      >
        <div className="space-y-6">
          ...
          <ModalActions>
            <ModalButton variant="secondary" onClick={calendarModal.close}>
              取消
            </ModalButton>
            <ModalButton 
              variant="primary" 
