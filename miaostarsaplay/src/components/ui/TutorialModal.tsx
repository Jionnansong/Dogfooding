...
import { ChevronRight, X, ArrowLeft, BookOpen, Sparkles, CheckCircle2, Zap } from 'lucide-react';
...
interface TutorialModalProps {
  ...
  onSelectTutorial: (tutorial: TutorialItem | null) => void;
  className?: string;
}
...
export const TutorialModal: React.FC<TutorialModalProps> = ({
  ...
  return (
    <div
      className={cn(
        '!m-0 fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/70 backdrop-blur-md animate-in fade-in duration-300',
        className
      )}
    >
      <div className="bg-white w-full h-full md:w-[90%] md:h-[85%] md:max-w-6xl md:rounded-[3rem] shadow-2xl animate-in slide-in-from-bottom-10 duration-500 flex flex-col overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 md:px-12 md:pb-16 custom-scrollbar relative">
          {!selectedTutorial ? (
            <TutorialListView tutorials={tutorials} onSelect={onSelectTutorial} />
          ) : (
            <TutorialDetailView tutorial={selectedTutorial} onBack={handleBack} />
          )}
        </div>
      </div>
    </div>
...
interface TutorialListViewProps {
  tutorials: TutorialItem[];
  onSelect: (tutorial: TutorialItem) => void;
}

const TutorialListView: React.FC<TutorialListViewProps> = ({ tutorials, onSelect }) => {
  return (
...
const TutorialDetailView: React.FC<TutorialDetailViewProps> = ({ tutorial, onBack }) => {
  return (
    <div className="max-w-4xl mx-auto space-y-10 md:space-y-12 animate-in fade-in slide-in-from-right-4 duration-500 pb-12">
      ...
      <div className="relative p-6 md:p-10 bg-indigo-600 rounded-[2rem] md:rounded-[3rem] text-white overflow-hidden shadow-xl shadow-indigo-100">
        <div className="relative z-10">
          ...
          <h4 className="text-2xl md:text-4xl font-black mb-4 tracking-tight leading-tight">{tutorial.title}</h4>
          <p className="text-indigo-100 font-medium leading-relaxed md:text-lg max-w-2xl">{tutorial.desc}</p>
        </div>
